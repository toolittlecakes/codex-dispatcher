...
Остаётся половина канала webview→host: каждый `postMessage` из шима — отдельный параллельный `fetch`, FIFO не гарантирован (см. ниже, доделывается отдельным коммитом).
