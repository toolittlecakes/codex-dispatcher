...
Сделано: любой завершившийся HTTP-обмен возвращается как `success` с реальным статусом и телом (`readExternalFetchBody`); `error` остался только для транспортных сбоев. Таймаут поднят 10s → 120s. По стримингу решение зафиксировано: host-стриминг не нужен — `fetch-stream-*` сообщения генерирует websocket-путь внутри самого вебвью, а контракт `fetch` — строго request/response.
